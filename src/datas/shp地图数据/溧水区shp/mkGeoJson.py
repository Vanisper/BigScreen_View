...
shpdata = GeoDataFrame.from_file("./LiShuiGeoJson.shp",encoding="utf-8")
...
for index,item in enumerate(features[0]["features"]):
    if "name" in item["properties"].keys():
        features[0]["features"][index]["properties"]["name"] = item["properties"]["name"]

    # 计算多边形质心
    from shapely.geometry import Polygon
    polygon = Polygon(features[0]["features"][index]["geometry"]["coordinates"][0][0] if features[0]["features"][index]["geometry"]["type"] =="MultiPolygon" else features[0]["features"][index]["geometry"]["coordinates"][0])
